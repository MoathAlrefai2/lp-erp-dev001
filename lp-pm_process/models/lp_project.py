...
class LP_Project(models.Model):
    ...
    def devops_sync(self):
        ...
        if wiql_results:
            ...
            tasks_to_be_inserted= [wit_client.get_work_item(int(tmp_devops_task.id)) for tmp_devops_task in wiql_results ]
            count_insert = 0
            count_updates = 0
            count_error_task = 0
            for work_item in tasks_to_be_inserted:
                ...
                if "Microsoft.VSTS.Common.StateChangeDate" in work_item.fields:
                    tmp_state_changedate = work_item.fields["Microsoft.VSTS.Common.StateChangeDate"][:19]
                    tmp_state_changedate=tmp_state_changedate.replace('T',' ')
                else:
                ...
                if "System.ChangedDate" in work_item.fields:
                    tmp_Changedate = work_item.fields["System.ChangedDate"][:19]
                    tmp_Changedate = tmp_Changedate.replace('T', ' ')
                ...
                if "System.AssignedTo" in work_item.fields:
                    tmp_assigned_to = self.search_user(self.id, work_item.fields["System.AssignedTo"])
                ...
                try:
                 if work_item.id not in tasks_to_be_synced:
                    ...
                    try:
                        ...
                        pass
                       # _logger.warning('WARNING: tags not valid on the task!')
                 else:

                  record_ids = self.env['project.task'].search([('lp_devops_ref_id', '=', work_item.id), ('project_id', '=', self.id)])

                  for record in record_ids:
                     if "System.ChangedDate" in work_item.fields:
                       changed_date = work_item.fields["System.ChangedDate"][:19]
                       changed_date =changed_date.replace('T',' ')
                       if str(record.lp_devops_changed_date) != changed_date: #or str(record.changed state) != changed_date
                    ...
                    _logger.warning('WARNING: task not sync succssfully!')
    ...
    def get_email(self, AssignedTo=''):
        ...
        try:
           ...
           # email = email[1: -1]
            ...
            _logger.warning('WARNING: email not valid on this task!')
        return email.strip("<>")


    def search_user(self, project_id, AssignedTo=''):
        email = self.get_email(AssignedTo)
        users = self.env['res.users'].sudo().search([('login', '=', email)])
        if users:
            return users[0].id
        else:
            project_id = self.env['project.project'].sudo().search([('id', '=', project_id)])
            if project_id:
                return project_id[0].user_id.id
            else:
                ''
